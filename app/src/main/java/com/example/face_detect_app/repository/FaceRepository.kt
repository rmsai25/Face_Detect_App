...
//    }
    ...
    suspend fun registerFace(name: String, bitmap: Bitmap): Result<RegisterResponse> {
        return withContext(Dispatchers.IO) {
            try {
                android.util.Log.d("FaceRepository", "Starting registration for: $name")
                android.util.Log.d("FaceRepository", "Bitmap size: ${bitmap.width}x${bitmap.height}")

                val file = bitmapToFile(bitmap, "face_register_${System.currentTimeMillis()}.jpg")
                android.util.Log.d("FaceRepository", "File created: ${file.length()} bytes")
                ...
                if (response.isSuccessful && response.body() != null) {
                    android.util.Log.d("FaceRepository", "Registration successful")
                    Result.success(response.body()!!)
                } else {
                    val errorBody = response.errorBody()?.string() ?: "Unknown error"
                    android.util.Log.e("FaceRepository", "Registration failed: $errorBody")
                    Result.failure(Exception(errorBody))
                }
            } catch (e: Exception) {
                android.util.Log.e("FaceRepository", "Exception: ${e.message}", e)
                Result.failure(e)
            }
        }
    }
